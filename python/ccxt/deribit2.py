...
class deribit2 (Exchange):
    ...
    def fetch_markets(self, params={}):
        ...
        currenciesResponse = self.publicGetGetCurrencies()
        currencies = self.safe_value(currenciesResponse, 'result')
        # Fetch instruments per currency
        for i in range(0, len(currencies)):
            instrumentsResponse = self.publicGetGetInstruments({'currency': currencies[i]['currency']})
            instruments = self.safe_value(instrumentsResponse, 'result')
            allInstruments.extend(instruments)
    ...
    def fetch_balance(self, params={}):
        ...
        for i in range(0, len(currencies)):
            ...
            accountSummaryResponse = self.privateGetGetAccountSummary({'currency': currentCurrency})
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'public':
            ...
            randomNonce = self.decode(self.random_nonce(16))
            # Craft relevant data
            requestData = method.upper() + '\n' + uri + '\n' + requestBody + '\n'
            stringToSign = timestamp + '\n' + randomNonce + '\n' + requestData
            signature = self.hmac(self.encode(stringToSign), self.encode(self.secret), hashlib.sha256)
            # Craft header
            headerVal = 'deri-hmac-sha256 ' + 'id=' + self.apiKey + ',ts=' + timestamp + ',nonce=' + randomNonce + ',sig=' + signature
            headers = {
